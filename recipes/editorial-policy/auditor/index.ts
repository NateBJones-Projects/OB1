...
import { storeDerivedThought } from "../_shared/derived-thought-writer.ts";
...
// Auth consolidated onto the brain-wide MCP_ACCESS_KEY (vault-backed, header
// pattern) — the old per-function AUDITOR_ACCESS_KEY leaked via ?key= URLs
// in cron commands/logs and is no longer accepted.
...
interface AuditResult {
  ...
  hygiene: Record<string, unknown> | null; // Tier 1 mechanical lint counts (never paged)
...
// Tier 1 lint (mechanical, SQL-only, free). Definitions live in the lint_*
// views; lint_hygiene_summary() aggregates their counts into one JSON. Read
// this BEFORE the LLM call so a flaky/costly model can never take the free
// hygiene metrics down with it. Returns null (not throw) on failure so the
// audit still proceeds.
...
async function callAuditor(
  ...
  // Some models (e.g. Anthropic via OpenRouter) wrap JSON in a ```json fence
  // even when response_format=json_object is requested. Strip it before parse.
...
async function storeAuditReport(result: AuditResult): Promise<string> {
  ...
  return await storeDerivedThought(supabase, content, {
    metadata: {
        ...
        hygiene: result.hygiene,   // Tier 1 mechanical lint counts (separate from findings)
...
Deno.serve(async (req: Request): Promise<Response> => {
  try {
    ...
    // Tier 1 hygiene is computed regardless of the LLM path and even on an
    // empty window, so mechanical metrics are recorded every run.
