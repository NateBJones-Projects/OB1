/**
 * Shared Claude CLI spawn utilities for the typed-edge-classifier recipe.
 *
 * ORIGIN: copied verbatim (modulo this header and the debug env-var name)
 * from `recipes/atomizer/lib/claude-cli.mjs`, which is where this pattern
 * was first worked out. Recipes in OB1 are self-contained folders that a
 * user can copy out on its own, and nothing in the repo imports across
 * recipe boundaries, so the file is duplicated rather than shared. If you
 * fix a bug here, fix it in the atomizer copy too.
 *
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        // contains arbitrary thought text from the user's brain. Set
        // CLASSIFY_EDGES_DEBUG=1 to include the raw snippets when debugging.
        const debug = process.env.CLASSIFY_EDGES_DEBUG === "1";
        const detail = debug
          ? `\nStderr: ${stderr.substring(0, 500)}\nStdout: ${stdout.substring(0, 200)}`
          : ` (stderr ${stderr.length}B, stdout ${stdout.length}B — set CLASSIFY_EDGES_DEBUG=1 to see)`;
