...
async function fetchThoughtReasoningEdges(sb, thoughtIds, limit = 200) {
  const uniq = Array.from(new Set((thoughtIds || []).filter((x) => x != null)));
  // Need at least 2 thoughts for an edge.
  if (uniq.length < 2) return [];

  // PostgREST `in.(...)` filter is safe here — thought ids are UUIDs
  // generated by the DB, not user-controlled input.
  const list = uniq.join(",");
  const query = [
    "select=from_thought_id,to_thought_id,relation,confidence,decay_weight,valid_from,valid_until,classifier_version,support_count,metadata,created_at",
    `from_thought_id=in.(${list})`,
    `to_thought_id=in.(${list})`,
    // Prefer high-confidence edges; fall back to support_count then recency.
    "order=confidence.desc.nullslast,support_count.desc.nullslast,created_at.desc",
    `limit=${limit}`,
  ].join("&");

  // Some brains may not have typed-reasoning-edges installed. Treat "table not found"
  // as a soft-miss instead of failing the whole wiki generation run.
  try {
    return (await sb.get("thought_edges", query)) || [];
  } catch (err) {
    const msg = String(err?.message || err);
    if (/\b(thought_edges)\b/i.test(msg) && /\b(404|not found|relation .* does not exist)\b/i.test(msg)) {
      return [];
    }
    throw err;
  }
...
function buildSynthesisInput(entity, linked, semantic, nameMap, maxLinked, maxSemantic, reasoningEdges) {
  ...
  // Thought-to-thought reasoning edges (from public.thought_edges), restricted
  // to the thought ids we are already sending as linked snippets. This keeps
  // the wiki grounded in evidence the model can actually see/cite.
  const thoughtMeta = new Map(linkedSnippets.map((t) => [t.id, { date: t.date, type: t.type }]));
  const reasoningByRelation = {};
  for (const e of reasoningEdges || []) {
    const rel = e.relation || "related_to";
    if (!reasoningByRelation[rel]) reasoningByRelation[rel] = [];
    const fromMeta = thoughtMeta.get(e.from_thought_id) || {};
    const toMeta = thoughtMeta.get(e.to_thought_id) || {};
    reasoningByRelation[rel].push({
      from_id: e.from_thought_id,
      to_id: e.to_thought_id,
      from_date: fromMeta.date ?? null,
      to_date: toMeta.date ?? null,
      from_type: fromMeta.type ?? null,
      to_type: toMeta.type ?? null,
  ...
  for (const rel of Object.keys(reasoningByRelation)) {
    reasoningByRelation[rel].sort((a, b) => {
      const bc = Number(b.confidence ?? 0);
      const ac = Number(a.confidence ?? 0);
      if (bc !== ac) return bc - ac;
      const bs = Number(b.support ?? 0);
      const as_ = Number(a.support ?? 0);
      if (bs !== as_) return bs - as_;
      return 0;
    });
    reasoningByRelation[rel] = reasoningByRelation[rel].slice(0, 20);
  }
  ...
  return {
    ...
    reasoning_edges_by_relation: reasoningByRelation,
...
If BOTH typed_edges_by_relation AND reasoning_edges_by_relation are empty, omit the Relationships section entirely.

Render two subsections:
### Entity relationships
- Use \`#### {relation_type}\` subheadings from typed_edges_by_relation (entity→entity edges).
- Under each relation, list other entities with support counts (and confidence if present).
- Do not render a co-mention subsection; co_occurs_with edges are excluded upstream.

### Thought reasoning links
- Use \`#### {relation_type}\` subheadings from reasoning_edges_by_relation (thought→thought edges).
- Under each relation, list links as: \`- [#from_id] {relation} [#to_id] (confidence=X, support=Y)\`
- These are links BETWEEN your evidence thoughts; they should help the reader trace why beliefs evolved.
...
async function generateForEntity(sb, env, entity, args) {
  ...
  // 4. Thought reasoning edges BETWEEN the linked evidence thoughts. Keep the
  // query set aligned to the thought ids we actually send to the model.
  let reasoning = [];
  try {
    const linkedForModel = (linked || []).slice(0, args.maxLinked).map((t) => t.id);
    reasoning = await fetchThoughtReasoningEdges(sb, linkedForModel, 200);
  } catch (err) {
    console.error(`[wiki] fetch thought reasoning edges failed for #${entity.id}: ${err.message}`);
  }

  console.log(
    `[wiki] #${entity.id} ${entity.canonical_name} (${entity.entity_type}): ` +
      `${linked.length} linked, ${eOut.length + eIn.length} typed edges, ` +
      `${reasoning.length} reasoning edges, ${semantic.length} semantic`,
  );
