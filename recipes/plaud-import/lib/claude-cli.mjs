/**
 * Shared Claude CLI spawn utilities for the plaud-import recipe.
 *
 * Copied verbatim from recipes/atomizer/lib/claude-cli.mjs so this recipe
 * folder stays self-contained when a user copies it to their own machine.
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        // contains arbitrary transcript text. Set PLAUD_DEBUG=1
        // to include the raw snippets when actively debugging.
        const debug = process.env.PLAUD_DEBUG === "1" || process.env.ATOMIZE_DEBUG === "1";
        const detail = debug
          ? `\nStderr: ${stderr.substring(0, 500)}\nStdout: ${stdout.substring(0, 200)}`
          : ` (stderr ${stderr.length}B, stdout ${stdout.length}B — set PLAUD_DEBUG=1 to see)`;
