    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-muted text-sm">
          Search across all entries, emails, and documents
        </p>
      </div>
          ...
          <div className="space-y-3">
            {state.results.map((r) => (
              <Link
                key={r.id}
                href={`/thoughts/${r.id}`}
                className="block bg-bg-surface border border-border rounded-lg p-4 hover:border-accent/30 transition-colors"
              >
