...
openclaw agent --session-id native-recheck-search --message "If tool openbrain_search is available, call it with query 'OpenBrain' limit 1 and reply exactly TOOL_OK. Otherwise reply exactly TOOL_MISSING." --json
MARKER="recheck-$(date -u +%Y%m%dT%H%M%SZ)-$RANDOM"
openclaw agent --session-id native-recheck-capture --message "Use tool openbrain_capture to capture content: ${MARKER}. category Decisions, tags platform:openclaw,topic:openbrain,kind:validation. Reply exactly CAPTURE_OK." --json
...
| `401/403` from MCP | wrong key/token, expired credential, wrong auth mode | verify URL key vs `config.apiKey`; rotate/update token | `openbrain_search` check + bundle |
...
| wrong tool names called by plugin | MCP server tool names differ from defaults | set `searchToolName` / `captureToolName` / `listToolName` in plugin config | `openbrain_search` and `openbrain_capture` tests |
...
| wrong MCP endpoint path/version | URL host reachable but route/protocol contract wrong | verify exact MCP endpoint path and minimal JSON-RPC probe request | direct probe + `openbrain_search` check |
...
| tool call fails with schema/argument error | invalid args passed to tool/MCP endpoint | validate payload shape and required fields, then retry with minimal known-good payload | direct `openbrain_search` with minimal args + bundle |
