...
Enable optional plugin tools with profile-safe additive policy (merge-safe):
...
try:
    ...
    cfg = {}
required=['openbrain-native','search_thoughts','capture_thought','list_thoughts','thought_stats','delete_thought']
...
Expected `kind` is `memory` and `toolNames` includes:
- `search_thoughts`
- `capture_thought`
- `list_thoughts`
- `thought_stats`
