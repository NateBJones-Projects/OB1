...
required={'search_thoughts','capture_thought','list_thoughts'}
if status!='loaded':
...
```bash
openclaw agent --session-id native-validate-search --message "If tool search_thoughts is available, call it with query 'OpenBrain' limit 1 and reply exactly TOOL_OK. Otherwise reply exactly TOOL_MISSING." --json > /tmp/openbrain-tool-check.json
python3 - <<'PY'
import json, sys
...
openclaw agent --session-id native-validate-capture --message "Use tool capture_thought to capture content: ${MARKER}. category Decisions, tags platform:openclaw,topic:openbrain,kind:validation. Reply exactly CAPTURE_OK." --json > /tmp/openbrain-capture-check.json
python3 - <<'PY'
import json, sys
